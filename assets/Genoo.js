...
{
    jQuery('body').append('<div class="WPMKTENGINELoading" id="WPMKTENGINELoading" style="display: none"><div class="WPMKTENGINELoader"><img src="'+ GenooVars.GenooPluginUrl +'logo.png"><div class="WPMKTENGINELoaderImage"></div><strong id="WPMKTENGINELoaderMessage">'+ message +'</strong></div></div>');
...
Api.loader.load = function()
{
    ...
    if(Api.data.key == true && Api.data.track == true){
        ...
        // Set values
        // Magic cycle
        jQuery.when(Api.set(Api.data.keyName, Api.data.keyData, 'update_option_api')).done(function(a1){
            ...
            jQuery.when(Api.set(Api.data.trackName, Api.data.trackData, 'update_option_api')).done(function(a2){
                // Set up leads
                Api.logMessage('Tracking updated via AJAX call, changing message...');
                jQuery('#WPMKTENGINELoaderMessage').text('Setting up leads.');
                jQuery.when(Api.set('update', 'true', 'update_leads')).done(function(a3){
                    // Donito done
                    Api.logMessage('Finished setup, changing message...');
                    jQuery('#WPMKTENGINELoaderMessage').text('Finished setting up your WordPress installation.');
                    setTimeout(function(){
...
Api.refreshForms = function()
{
    ...
    jQuery.when(Api.set('refreshForms', 'true', 'refresh_forms')).done(function(a1){
        Api.loader.close();
    });
};
...
Api.refreshSurveys = function()
{
    ...
    jQuery.when(Api.set('refreshSurveys', 'true', 'refresh_surveys')).done(function(a1){
        Api.loader.close();
    });
};
...
Api.listener = function(event)
{
    ...
    if(event.origin !== "https:" + GenooVars.DOMAIN){ console.log('Wrong origin: ' + event.origin); return; }
    // Event Data
    if('name' in event.data && 'value' in event.data){
        ...
        switch(event.data.name){
            ...
            case 'trackingkey':
                ...
                Api.logMessage('Api.set(\''+ event.data.name +'\', \''+ event.data.value +'\', \'update_option_api\')');
 ...
 * @param url
 */
Tool.promptGo = function(question, url)
{
...
Genoo.startCTAsImport = function(event)
{
    ...
    var data = { action: 'wpme_import_cta_count'};
    ...
    jQuery.post(ajaxurl, data, function(response){
        ...
        if(response.error){
            ...
            (function importCTAS(){
                ...
                msgOffset = msgStep * msgPer;
                var temp = { action: 'wpme_import_ctas', offset: msgOffset, per: msgPer };
...
WPMESlide.goTo = function(element, event, target, dontblock)
{
    ...
    Tool.removeClass(document.getElementById('wpme_row_id_' + intTarget), 'hidden');
    // Check radio!
